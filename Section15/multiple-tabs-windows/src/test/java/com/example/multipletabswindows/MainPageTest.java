...
public class MainPageTest {
    ...
    */
        ...
        // now that the desired input value is in a string variable
        // we need to put it in the input field of the original window

        // switch back to the original window and verify we can see the input
        driver.switchTo().window(originalWindowHandle);
        WebElement nameInput = wait.until(
                ExpectedConditions.visibilityOf(mainPage.nameInput)
        );
        Assert.assertNotNull(nameInput);

        // type in the desired name in the input field and
        // verify the input field has capture it
        nameInput.sendKeys(firstCourseTextDescription);
        Assert.assertEquals(nameInput.getAttribute("value"),
                firstCourseTextDescription);
    }
}
